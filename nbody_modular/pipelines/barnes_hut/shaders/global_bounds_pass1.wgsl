...
struct BoundsBuffer {
    data: array<vec4<f32>>, // min, max for each workgroup
};
...
@group(0) @binding(1) var<storage, read_write> partial_bounds: BoundsBuffer;
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>, @builtin(workgroup_id) wgid: vec3<u32>) {
    ...
    if (i < params.particleCount) {
        my_min = particles.data[i].xyz;
        my_max = particles.data[i].xyz;
    }
    ...
    if (lid.x == 0) {
        partial_bounds.data[wgid.x * 2] = vec4<f32>(local_min[0], 0.0);
        partial_bounds.data[wgid.x * 2 + 1] = vec4<f32>(local_max[0], 0.0);
    }
}
